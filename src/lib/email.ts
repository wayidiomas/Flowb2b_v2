...
export async function sendEmailConfirmation(
  ...
  const html = `
    ...
    <body style="font-family: 'Inter', Arial, sans-serif; background-color: #f5f5f5; margin: 0; padding: 20px;">
      <div style="max-width: 600px; margin: 0 auto; background-color: #ffffff; border-radius: 16px; overflow: hidden; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
        ...
        <!-- Content -->
        <div style="padding: 40px 30px;">
          <h2 style="color: #2660a5; font-size: 24px; margin: 0 0 20px 0;">Bem-vindo ao FlowB2B, ${nome || 'usuário'}!</h2>

          <p style="color: #475467; font-size: 16px; line-height: 24px; margin: 0 0 20px 0;">
            Obrigado por se cadastrar! Para começar a usar a plataforma, confirme seu email clicando no botão abaixo:
          </p>

          <div style="text-align: center; margin: 30px 0;">
            <a href="${confirmationUrl}" style="display: inline-block; background-color: #ffbe4a; color: #ffffff; font-size: 16px; font-weight: 600; text-decoration: none; padding: 14px 32px; border-radius: 8px;">
              Confirmar meu email
            </a>
          </div>
          ...
          <p style="color: #667085; font-size: 14px; line-height: 20px; margin: 0;">
            Se você não criou uma conta no FlowB2B, ignore este email.
          </p>
          ...
          <p style="color: #9ca3af; font-size: 12px; line-height: 18px; margin: 0;">
            Se o botão não funcionar, copie e cole o link abaixo no seu navegador:<br>
            <a href="${confirmationUrl}" style="color: #2660a5; word-break: break-all;">${confirmationUrl}</a>
          </p>
        </div>
  ...
  return sendEmail({
    to: email,
    subject: 'Confirme seu email - FlowB2B',
    html,
  })
}
