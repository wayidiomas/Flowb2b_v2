...
const navItems = [
  ...
  { href: '/representante/catalogo', label: 'Catalogo', icon: CatalogoIcon },
  { href: '/representante/landing-pages', label: 'Landing Pages', icon: LandingIcon },
...
export function RepresentanteLayout({ children }: RepresentanteLayoutProps) {
  const { user, logout } = useRepresentanteAuth()
    ...
    <div className="min-h-screen bg-gray-50 overflow-x-hidden">
      ...
      <header className="bg-[#336fb6] h-[60px] w-full shadow-md z-50 relative">
        <div className="h-full flex items-center px-6 md:px-12 gap-6">
          {/* Logo FlowB2B + Badge Representante */}
          <Link href="/representante/dashboard" className="shrink-0 flex items-center gap-3">
            <Image
              ...
              priority
            />
            <span className="hidden sm:inline-flex text-xs bg-violet-500 text-white px-2.5 py-1 rounded-full font-semibold shadow-sm">
              Representante
            </span>
          </Link>
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="w-8 h-8 rounded-full bg-white/20 flex items-center justify-center overflow-hidden">
                <span className="text-white text-sm font-medium">
                  {user?.nome?.charAt(0).toUpperCase() || 'R'}
                </span>
              </div>
              {/* Info */}
              <div className="text-left hidden md:block">
                <p className="text-white text-sm font-medium leading-tight">
                  {user?.nome || 'Representante'}
                </p>
