...
function NovaConferenciaEstoqueContent() {
  const { loading: authLoading } = useRepresentanteAuth()
  ...
  useEffect(() => {
    if (!empresaId || !fornecedorId) return
    const criarConferencia = async () => {
      try {
        const res = await fetch('/api/representante/conferencia-estoque', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ empresa_id: Number(empresaId), fornecedor_id: Number(fornecedorId) }),
        })
        if (res.ok) {
  ...
  const handleBipar = async (ean: string, quantidade: number, modo?: 'somar' | 'substituir') => {
    ...
    try {
      ...
      const res = await fetch(`/api/representante/conferencia-estoque/${conferenciaId}/itens`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      // Verificar duplicado ANTES de !res.ok (API retorna 200 com duplicado=true)
  ...
  const fetchItens = async () => {
    ...
    try {
      const res = await fetch(`/api/representante/conferencia-estoque/${conferenciaId}`)
      if (res.ok) {
        const data = await res.json()
        setItens(data.itens || [])
      }
  ...
  const handleRemoverItem = async (itemId: number) => {
    if (!conferenciaId) return
    try {
      const res = await fetch(`/api/representante/conferencia-estoque/${conferenciaId}/itens?item_id=${itemId}`, {
        method: 'DELETE',
      })
      if (res.ok) {
  ...
  const handleEnviar = async () => {
    ...
    setEnviando(true)
    try {
      const res = await fetch(`/api/representante/conferencia-estoque/${conferenciaId}/enviar`, {
        method: 'POST',
      })
      if (res.ok) {
        router.push(`/representante/conferencia-estoque/${conferenciaId}`)
      } else {
        const data = await res.json().catch(() => ({}))
    ...
    return (
      <RepresentanteLayout>
        <div className="max-w-2xl mx-auto text-center py-12">
          <p className="text-gray-500">Parametros empresa_id e fornecedor_id nao informados.</p>
          <Button
            variant="outline"
            className="mt-4"
            onClick={() => router.push('/representante/conferencia-estoque')}
          >
            Voltar
          </Button>
        </div>
...
export default function NovaConferenciaEstoquePage() {
  return (
    <Suspense fallback={<RepresentanteLayout><Skeleton className="h-96" /></RepresentanteLayout>}>
