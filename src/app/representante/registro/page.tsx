...
function RepresentanteRegistroForm() {
  ...
  // Se tem codigo de convite na URL, redirecionar para fluxo de convite
  ...
  const handleCnpjChange = (value: string) => {
    ...
    // Reset state when user changes CNPJ
    if (digits.length < 14) {
      setFornecedorEncontrado(null)
      setCnpjError('')
    }

    // Auto-validate when 14 digits
    if (digits.length === 14) {
      validarCnpj(digits)
    }
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!fornecedorEncontrado) {
      setError('CNPJ nao encontrado. Verifique se o CNPJ esta correto.')
      return
    }
    ...
    try {
      const res = await fetch('/api/auth/representante/registro', {
        method: 'POST',
      ...
      if (data.success) {
        ...
        setTimeout(() => router.push('/representante/dashboard'), 1500)
      } else {
  ...
  // Se tem codigo, mostrar loading enquanto redireciona
  if (codigo) {
    return (
  ...
  return (
    <RepresentanteAuthLayout description="Cadastre-se como representante e gerencie pedidos de todos os seus fornecedores.">
      <Card shadow="lg" rounded="xl" className="w-full max-w-[420px]">
        <CardContent className="p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {/* CNPJ com validacao em tempo real */}
            <div>
              <Input
                ...
                hint="CNPJ do fornecedor que voce representa"
                rightIcon={
                  validandoCnpj ? (
                    <SpinnerIcon />
              ...
              />
              {/* Badge de fornecedor encontrado */}
              {fornecedorEncontrado && (
            ...
            <Input
              ...
              hint="A senha deve ter no minimo 6 caracteres"
              rightIcon={
                <button
                  type="button"
