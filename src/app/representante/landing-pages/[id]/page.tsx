...
import type { LandingPageRepresentante, LpModo, LandingPageRepresentanteProduto } from '@/types/landing-page'

export default function LandingPageRepDetalhePage() {
  const params = useParams()
  const router = useRouter()
  const lpId = Number(params.id)

  const [lp, setLp] = useState<LandingPageRepresentante | null>(null)
  const [produtos, setProdutos] = useState<LandingPageRepresentanteProduto[]>([])
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = await fetch(`/api/representante/landing-pages/${lpId}`)
        if (res.ok) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const refresh = await fetch(`/api/representante/landing-pages/${lpId}`)
      if (refresh.ok) {
  ...
  const handleDelete = async () => {
    setDeleting(true)
    try {
      const res = await fetch(`/api/representante/landing-pages/${lpId}`, { method: 'DELETE' })
      if (res.ok) {
        router.push('/representante/landing-pages')
      } else {
        const data = await res.json()
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-6 space-y-5">
          ...
          <div>
            ...
            {form.modo === 'selecao' && (
              <div className="mt-3 rounded-xl border border-amber-200 bg-amber-50 p-3 text-xs text-amber-800">
                <p className="font-medium mb-1">Curadoria de produtos</p>
                <p>
                  Atualmente voce tem <strong>{produtos.length}</strong> produtos
                  curados nessa LP. A curadoria detalhada esta na proxima sprint —
                  hoje use o modo &quot;Todos&quot; ou &quot;Comprados&quot; para auto-popular.
                </p>
              </div>
            )}
          </div>
          ...
          <div className="border-t border-gray-100 pt-5">
            ...
            <ImageUploader
              ...
              label="Seu logo / foto"
              hint="PNG ou JPG, max 2MB. Aparece no card do hero."
            ...
            <div className="mt-4">
              <ImageUploader
                ...
                hint="PNG ou JPG, max 2MB. Vira fundo do hero."
                aspect="aspect-[4/1]"
          ...
          <div className="border-t border-gray-100 pt-5">
            ...
            <div>
              ...
              <textarea
                ...
                rows={4}
                placeholder="Conte um pouco sobre voce como representante..."
...
function ImageUploader({
  ...
  const handleUpload = async (file: File) => {
    ...
    try {
      ...
      fd.append('file', file)
      fd.append('kind', kind)
      const res = await fetch(`/api/representante/landing-pages/${lpId}/upload`, {
        method: 'POST',
        body: fd,
      })
      const data = await res.json()
  ...
  const handleRemove = async () => {
    setUploading(true)
    try {
      const res = await fetch(`/api/representante/landing-pages/${lpId}/upload?kind=${kind}`, {
        method: 'DELETE',
      })
