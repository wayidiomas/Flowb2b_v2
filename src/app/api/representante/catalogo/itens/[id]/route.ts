...
export async function PUT(
  ...
  try {
    ...
    try {
      ...
      console.error('Erro ao notificar lojistas (nao bloqueante, edicao manual representante):', notifyErr)
    }
    ...
    console.error('Erro ao atualizar item do catalogo (representante):', error)
...
export async function DELETE(
  ...
  try {
    ...
    try {
      ...
      console.error('Erro ao notificar lojistas (nao bloqueante, delete representante):', notifyErr)
    }

    return NextResponse.json({ success: true, deleted: count || 0 })
  } catch (error) {
    console.error('Erro no DELETE item (representante):', error)
    return NextResponse.json({ error: 'Erro interno' }, { status: 500 })
  }
}
