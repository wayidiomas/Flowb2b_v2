...
export async function POST(request: NextRequest) {
  try {
    ...
    // Para o job, usamos representanteUserId como fornecedor_user_id (campo é numerico)
    const { data: job, error: jobErr } = await supabase
      .from('catalogo_import_xlsx_jobs')
      .insert({
        catalogo_id: catalogo.id,
        fornecedor_user_id: Number(user.representanteUserId),
        cnpj: cnpjLimpo,
        status: 'pending',
    ...
    console.error('Erro na importacao (representante):', error)
