...
// POST - Representante saves motivo_faltante / previsao_retorno on validation items
export async function POST(
  ...
  try {
    ...
    // Get representante user name for tracking
    ...
    const validadoPor = representanteUser?.nome || user.email
      ...
      .upsert(
        {
          ...
          validado_por: `${validadoPor} (representante)`,
          status: 'validado',
          total_ok: totalOk,
    ...
    if (upsertError || !validacao) {
      console.error('Erro ao salvar validacao (representante disponibilidade):', upsertError)
    ...
    if (itens.length > 0) {
      ...
      if (insertError) {
        console.error('Erro ao inserir itens da validacao (representante):', insertError)
    ...
    let descricaoTimeline = 'Representante informou disponibilidade dos itens faltantes'
    const partes: string[] = []
    ...
    console.error('Erro ao salvar disponibilidade (representante):', error)
