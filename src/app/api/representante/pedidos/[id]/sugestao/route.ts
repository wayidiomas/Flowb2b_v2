...
export async function POST(
  ...
  try {
    ...
    for (const item of itens) {
      ...
      // casar/nomear o item (evita orfaos "Produto sem nome").
    ...
    // Validar acesso: representantes vinculados ao usuario
    ...
    // Verificar que o pedido existe e pertence ao representante
    ...
    const { data: sugestao, error: sugestaoError } = await supabase
      ...
      .insert({
        ...
        fornecedor_user_id: null, // representante nao tem fornecedor_user_id
    ...
    if (sugestaoError || !sugestao) {
      console.error('Erro ao criar sugestao (representante):', sugestaoError)
      return NextResponse.json({ error: 'Erro ao criar sugestao' }, { status: 500 })
    }

    // Buscar valores originais dos itens do pedido (snapshot para diff 3-vias / auditoria)
    const itemIds = itens
      .map(i => i.item_pedido_compra_id)
    ...
    const sugestaoItens = itens.map(item => {
      ...
      return {
        ...
        bonificacao_quantidade: item.bonificacao_quantidade || 0,
    ...
    if (itensError) {
      console.error('Erro ao inserir itens da sugestao (representante):', itensError)
    }
    ...
    // Buscar nome do usuario representante
    ...
    await supabase
      ...
      .insert({
        ...
        descricao: observacao
          ? `Sugestao enviada pelo representante: "${observacao}"`
    ...
    console.error('Erro ao enviar sugestao (representante):', error)
...
export async function DELETE(
  ...
  try {
    ...
    // Verificar que a sugestao existe, pertence a este pedido e foi feita por representante
    const { data: sugestao, error: sugestaoError } = await supabase
      .from('sugestoes_fornecedor')
    ...
    if (sugestao.autor_tipo !== 'representante') {
      return NextResponse.json(
        { error: 'Apenas sugestoes de representante podem ser excluidas por aqui' },
        { status: 403 }
      )
    }
    ...
    // Buscar nome do usuario representante para a timeline
    const { data: representanteUser } = await supabase
      .from('users_representante')
      .select('nome')
      .eq('id', user.representanteUserId)
    ...
    console.error('Erro ao excluir sugestao (representante):', error)
    return NextResponse.json({ error: 'Erro interno' }, { status: 500 })
  }
}
