...
// POST - Representante aceita ou rejeita contra-proposta do lojista
export async function POST(
  ...
  try {
    ...
    // Buscar fornecedores vinculados aos representantes
    ...
    const { data: pedido } = await supabase
      .from('pedidos_compra')
      .select('id, status_interno, bling_id, situacao, empresa_id, fornecedor_id')
      .eq('id', pedidoId)
    ...
    const autorNome = representanteUser?.nome || user.email
    ...
    if (action === 'aceitar') {
        ...
        await supabase
          ...
          .insert({
            ...
            descricao: observacao
              ? `Representante aceitou contra-proposta: "${observacao}"${blingSyncSuccess ? ' (Bling: Em Andamento)' : ''}`
              : `Representante aceitou a contra-proposta do lojista${blingSyncSuccess ? ' (Bling: Em Andamento)' : ''}`,
      ...
      void logActivity({
        ...
        empresaId: null,
        metadata: { pedido_id: pedidoId, tipo: 'resposta_contra_proposta' },
      }).catch(() => {})
    ...
    if (action === 'rejeitar') {
      ...
      // Buscar a ultima sugestao do representante (rejeitada quando lojista enviou contra-proposta)
      // e reativar ela para que o lojista possa aceitar ou manter original
      const { data: ultimaSugestaoRepresentante } = await supabase
        .from('sugestoes_fornecedor')
        .select('id')
        .eq('pedido_compra_id', pedidoId)
      ...
      if (ultimaSugestaoRepresentante) {
        // Reativar a sugestao
        await supabase
          .from('sugestoes_fornecedor')
          .update({
          ...
          .eq('id', ultimaSugestaoRepresentante.id)
      }

      // Voltar para sugestao_pendente
      await supabase
        .from('pedidos_compra')
      ...
      await supabase
        ...
        .insert({
          ...
          descricao: observacao
            ? `Representante rejeitou contra-proposta: "${observacao}" - Negociacao encerrada`
            : 'Representante rejeitou a contra-proposta - Negociacao encerrada',
          autor_tipo: 'representante',
          autor_nome: autorNome,
        })
