...
import type { FornecedorListItem } from '@/types/fornecedor'
...
export default function FornecedoresPage() {
  ...
  const fetchFornecedores = async () => {
    ...
    try {
      ...
      // Usar a view fornecedores_produtos_view que já tem produtos_vinculados calculado
      const { data, error } = await supabase
        .from('fornecedores_produtos_view')
        .select('*')
      ...
      if (data && data.length > 0) {
        ...
        const fornecedoresFormatados: FornecedorListItem[] = data.map(f => ({
          id: f.fornecedor_id,
          nome: f.fornecedor_nome || '',
          cnpj: f.fornecedor_cnpj,
          telefone: f.fornecedor_contato,
  ...
  const filteredFornecedores = fornecedores.filter((forn) => {
    ...
    const matchesSearch =
      forn.nome?.toLowerCase().includes(searchLower) ||
      forn.nome_fantasia?.toLowerCase().includes(searchLower) ||
      forn.cnpj?.includes(searchTerm) ||
      forn.cpf?.includes(searchTerm)

    // Filtro por CNPJ/CPF
    const docFilterClean = cnpjFilter.replace(/\D/g, '')
    const fornCnpjClean = forn.cnpj?.replace(/\D/g, '') || ''
  ...
  const handleSelectAll = () => {
    if (selectedFornecedores.length === paginatedFornecedores.length) {
      setSelectedFornecedores([])
    } else {
      setSelectedFornecedores(paginatedFornecedores.map((f) => f.id))
    }
  }

  const handleSelectFornecedor = (id: number) => {
  ...
  const handleExcluir = async () => {
    ...
    if (!confirm(`Deseja realmente excluir ${selectedFornecedores.length} fornecedor(es)?`)) return
  ...
  const formatContato = (telefone?: string, celular?: string) => {
    return telefone || celular || '-'
  }

  // Formatar CNPJ/CPF
  const formatDocumento = (cnpj?: string, cpf?: string) => {
    if (cnpj) {
  ...
  return (
    ...
    <DashboardLayout>
      ...
      <PageHeader
        title="Fornecedores"
        subtitle={`${filteredFornecedores.length} fornecedores`}
      />
      ...
      <div className="bg-white rounded-[20px] shadow-[0px_0px_12.4px_1px_rgba(137,170,255,0.1)] overflow-hidden">
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead>
              <tr className="border-b border-[#EFEFEF]">
                <th className="px-4 py-3 text-left w-12">
                  <input
                    type="checkbox"
                    checked={paginatedFornecedores.length > 0 && selectedFornecedores.length === paginatedFornecedores.length}
            ...
            <tbody>
              {loading ? (
                <TableSkeleton columns={4} rows={5} showCheckbox showActions />
                ...
                paginatedFornecedores.map((forn, index) => (
                  <tr
                    key={forn.id}
                    className={`border-b border-[#EFEFEF] hover:bg-gray-50 ${
                      selectedFornecedores.includes(forn.id) ? 'bg-blue-50' : index % 2 === 1 ? 'bg-[#F9F9F9]' : 'bg-white'
                    }`}
                  >
                    <td className="px-4 py-3">
                      <input
                        ...
                        onChange={() => handleSelectFornecedor(forn.id)}
                        className="w-5 h-5 text-[#336FB6] bg-white border-[#DCDCDC] rounded focus:ring-[#336FB6]"
                      />
                    </td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3">
                      <span className="text-[13px] text-[#344054]">
                        {formatDocumento(forn.cnpj, forn.cpf)}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                      <span className="text-[13px] text-[#344054]">
                        {formatContato(forn.telefone, forn.celular)}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-center">
                      <span className="text-[13px] text-[#344054]">
                        {forn.produtos_vinculados || 0}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-center">
                      <Link
