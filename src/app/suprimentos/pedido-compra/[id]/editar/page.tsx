...
export default function EditarPedidoCompraPage() {
  ...
  const [politicaSelecionada, setPoliticaSelecionada] = useState<number | null>(null)
    ...
    const fetchPedido = async () => {
      ...
      try {
        ...
        if (pedido && pedido.length > 0) {
          ...
          setDataPrevista(p.data_prevista ? p.data_prevista.split('T')[0] : '')
          setDesconto(p.desconto || 0)
          setFrete(p.frete || 0)
          setTotalIcms(p.total_icms || 0)
          setTransportador(p.transportador || '')
          setFretePorConta(p.frete_por_conta || 'CIF')
          setObservacoes(p.observacoes || '')
          setObservacoesInternas(p.observacoes_internas || '')
          setSituacao(p.situacao || 5)
          setPoliticaSelecionada(p.politica_id || null)
    ...
    <DashboardLayout>
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-3">
              <h1 className="text-xl font-semibold text-[#344054]">
        ...
        <div className="flex items-center gap-2">
          ...
          <select
            value={situacao}
            onChange={(e) => setSituacao(Number(e.target.value) as SituacaoPedido)}
                ...
                <div>
                  {politicas.length === 0 ? (
                    ...
                    <div className="space-y-4">
                      {politicas.map(pol => {
                        const isSelected = politicaSelecionada === pol.id
                        const isApplicable = totalProdutos >= (pol.valor_minimo || 0)
                        ...
                        return (
                          <div
                            ...
                            onClick={() => isApplicable && setPoliticaSelecionada(isSelected ? null : pol.id)}
                          >
                              ...
                              <div className="flex-1">
                                ...
                                <div className="grid grid-cols-2 gap-4 mt-3 text-sm">
                                  ...
                                  <div>
                                    ...
                                    <span className="ml-2 font-medium">{pol.desconto || 0}%</span>
                                  </div>
                                  <div>
                                    <span className="text-gray-500">Bonificacao:</span>
                                    <span className="ml-2 font-medium">{pol.bonificacao || 0}%</span>
                                  </div>
                                  <div>
                                    <span className="text-gray-500">Prazo de entrega:</span>
                                    <span className="ml-2 font-medium">{pol.prazo_entrega || 0} dias</span>
                                  </div>
                                  {pol.forma_pagamento_dias && pol.forma_pagamento_dias.length > 0 && (
                                    <div className="col-span-2">
                                      <span className="text-gray-500">Dias de pagamento:</span>
                                      <span className="ml-2 font-medium">{pol.forma_pagamento_dias.join(', ')} dias</span>
                                    </div>
                                  )}
                                </div>
                              </div>
                      ...
                      <table className="w-full">
                        <thead>
                          <tr className="border-b border-gray-200">
                            ...
                            <th className="px-3 py-2 text-left text-xs font-medium text-gray-500">Data Vencimento</th>
                            ...
                            <tr key={index} className="border-b border-gray-100 hover:bg-gray-50">
                              ...
                              <td className="px-3 py-2">
                                <input
                                  ...
                                  className="w-32 px-2 py-1 text-sm border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-[#336FB6]"
                                />
                              </td>
                              <td className="px-3 py-2">
                                <input
